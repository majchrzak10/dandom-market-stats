...
export function dedupeCompetitorOffers({ otodom, olx, no = [] }) {
  ...
  // Plus: NO ma sporo wewnetrznych duplikatow (ta sama oferta wystawiona przez
  // roznych agentow = rozne externalId, ten sam city+area+price). Wciagamy je tez
  // przez findKeyMatch, ktory teraz patrzy na CALY unique pool wlacznie z dodanymi NO.
  let noInternalDupes = 0;
  for (const n of no) {
    ...
    if (matched != null) {
      if (!unique[matched].sources.includes("nieruchomosci-online")) {
        ...
        // Mamy juz NO oferte z tym key - to wewnetrzny duplikat NO. Liczymy ID jako alias.
        noInternalDupes++;
        if (!unique[matched].noAliases) unique[matched].noAliases = [];
        unique[matched].noAliases.push(n.externalId);
        duplicates.push({ noId: n.externalId, matched: unique[matched].externalId, via: "internal-NO" });
      }
      continue;
    }
    // Nowa unique oferta NO - dodaj do unique I do indexu zeby kolejne NO mogly sie z nia match'owac
    const newIdx = unique.length;
    unique.push({ ...n, sources: ["nieruchomosci-online"] });
    const k = buildMatchKey(n);
    if (k) {
      if (!matchKeyToIndex.has(k)) matchKeyToIndex.set(k, []);
      matchKeyToIndex.get(k).push(newIdx);
    }
  }
